#' @title Predict H1 open for each season
#' @export
#' @author Jack G Hendrix

predict_h1_open_seasonal <- function(DT, model, predictor, season_key)
	{
	...
	new_data <-		DT[, .(
			sl_ = mean(sl_),
			forest = 0,
			open = seq(from = 0, to = 1, length.out = 100L),
	...
	new_data[, paste0('h1_open_s_', predictor) := predict(model, .SD, type = 'link', re.form = NULL)]
